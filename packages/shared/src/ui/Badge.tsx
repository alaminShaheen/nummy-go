...
const STATUS_CONFIG: Record<
	Uppercase<OrderStatus>,
	{ variant: BadgeProps['variant']; label: string }
> = {
	PENDING: {variant: 'info', label: 'Pending'},
	PREPARING: {variant: 'warning', label: 'Preparing'},
	READY: {variant: 'success', label: 'Ready'},
	COMPLETED: {variant: 'secondary', label: 'Delivered'},
	CANCELLED: {variant: 'destructive', label: 'Cancelled'},
	ACCEPTED: {variant: 'success', label: 'Success'},
};

export function OrderStatusBadge({status, className}: { status: OrderStatus; className?: string }) {
	const {variant, label} = STATUS_CONFIG[status.toUpperCase() as Uppercase<OrderStatus>];
	return (
