...
interface TenantNewOrderEmailProps {
  tenantName: string;
  orderId: string;
  createdAt: number;
  totalCents: number;
...
export function TenantNewOrderEmail({
  ...
  return (
    <NummyGoEmailLayout previewText={`New order #${orderId.slice(-6).toUpperCase()} received from ${customerName}`}>

      <Text style={s.h1}>New Order Received 🛎️</Text>
      <Text style={s.subtitle}>
        A customer just placed an order at <strong>{tenantName}</strong>. Open your NummyGo dashboard to accept and begin preparation.
      </Text>

      {/* Customer info */}
      <Section style={s.infoBox}>
        <Text style={s.infoLabel}>CUSTOMER INFORMATION</Text>
      ...
      <Section style={s.receipt}>
        <Text style={s.receiptTitle}>Order Details</Text>

        <Row style={s.metaRow}>
          <Column>
            <Text style={s.metaLabel}>Time Received</Text>
          ...
          <Column>
            ...
            <Text style={{ ...s.metaValue, color: '#f59e0b' }}>Pending</Text>
          </Column>
        </Row>
        ...
        <Row>
          <Column><Text style={s.totalLabel}>Expected Revenue</Text></Column>
          <Column style={{ textAlign: 'right' }}>
...
const s: Record<string, React.CSSProperties> = {
  h1: { fontSize: '26px', fontWeight: '800', color: '#10b981', margin: '0 0 8px 0' },
